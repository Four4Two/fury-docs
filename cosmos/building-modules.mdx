...
In this tutorial we will be going over building a module in Highbury to show how easy it is to build on top of the Blackfury ecosystem. This module will be simple in nature but will show how to set up and connect a module to Highbury and can be used as a starting point for more complex modules.

## Set up

```
This tutorial is based on the v44 cosmos version which Fury is currently migrating to, to follow this tutorial clone the fury repo but ensure you
clone the upgrade-v44 branch as the master branch is currently on v39 & ensure you have futool, docker & go installed on your machine.
git clone -b upgrade-v44 https://github.com/Four4Two/fury.git
```

## Defining Protocol Buffer Types

The first step in building a new Fury Module is to define our Module's types. To do that we use Protocol Buffers which is a used for serializing structured data and generating code for multiple target languages, Protocol Buffers are also smaller than JSON & XML so sending data around the network will be less expensive. [Learn More](https://developers.google.com/protocol-buffers).

Our Protobuf files will all live in `proto/fury` directory. we will create a new directory with the new module `greet` and add the following files in the `proto/greet/v1beta1/` directory
...
```
syntax = "proto3";
package  fury.greet.v1beta1;
import  "cosmos_proto/cosmos.proto";
import  "gogoproto/gogo.proto";
option  go_package = "github.com/four4two/fury/x/greet/types";
...
import  "cosmos_proto/cosmos.proto";
import  "fury/greet/v1beta1/greet.proto";

service  Query {
	rpc  Greet(QueryGetGreetRequest) returns (QueryGetGreetResponse) {
	option  (google.api.http).get = "/fury/greet/v1beta1/greetings/{id}";
	}
	rpc  GreetAll(QueryAllGreetRequest) returns (QueryAllGreetResponse) {
	option  (google.api.http).get = "/fury/swap/v1beta1/greetings";
	}
}
...
Once all the files are filled in we are ready to generate our proto types. in the Fury Directory run `make proto-gen ` to generate the types, this will create a folder inside the `x/greet` and will contain the auto-generated proto types.
...
import (
	...
	"github.com/four4two/fury/x/greet/types"
	sdkerrors  "github.com/cosmos/cosmos-sdk/types/errors"
)
...
import (
	...
	"github.com/four4two/fury/x/greet/client/cli"
	"github.com/four4two/fury/x/greet/keeper"
	"github.com/four4two/fury/x/greet/types"
	"github.com/spf13/cobra"
	abci  "github.com/tendermint/tendermint/abci/types"
)
...
1. inside the root of our directory run `docker build -t fury/fury:tutorial-demo .`
2. find the directory for `futool` and open in your favorite code editor
3. run `futool testnet gen-config fury --fury.configTemplate upgrade-v44` which will create a bunch of files inside `full_configs/generated`
4. open up the two `docker-compose.yaml` files the one inside `generated` & the one inside `generated/fury` and change the image to point to `fury/fury:tutorial-demo` this will point to the local image we just built
5. change into the `full_configs/generated` directory and run `docker compose up -d`
6. now run `docker compose exec furynode bash` to bash into our `fury` cli inside the running container
...
`fury tx greet create-greeting "hello world from fury chain" --from whale`

now let's test to see if the greeting message is able to be queried:

`fury q greet list-greetings`
...
greetings:
- id: "0"
  message: hello world from fury chain
  owner: fury173w2zz287s36ewnnkf4mjansnthnnsz7rtrxqc
pagination: null
```

Now let's test if we can query the greeting by it's id which in our case will be `"0"`, run the following:

`fury q greet get-greeting 0`
