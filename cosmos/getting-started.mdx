...
This document will help you get a fury network up and running locally on your machine. It uses a CLI tool call `futool` that manages the configuration and initial setup of a network. The tool uses docker containers to run a Fury network with some optional configuration.
...
`futool` has a lot more functionality than simply running a single local fury network. Be sure to use its `--help` command and check out the repo's [README.md](https://github.com/Four4Two/futool/blob/master/README.md).
