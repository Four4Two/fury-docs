...
Client software for running an oracle on the fury blockchain. Currently tested against fury-testnet-11000.
...
Setup a fury blockchain node with a REST endpoint. See [here](https://medium.com/four4two/fury-rest-server-guide-a13bdecfc5e4) for instructions.
...
Configure a `.env` file in `fury-tools/scripts` like the one below:

```
# the chain-id
CHAIN_ID="fury-testnet-11000"
...
MARKET_IDS="bnb:usd,bnb:usd:30,btc:usd,btc:usd:30,xrp:usd,xrp:usd:30,busd:usd,busd:usd:30,fury:usd,fury:usd:30,jinx:usd,jinx:usd:30,usdf:usd"
...
# if the oracle should use the legacy HD path for fury (118). Setting to "true" will use 118, false will use 459
LEGACY_HD_PATH="false"

# if the oracle should include a fee when posting prices (in integer ufury units). Example will pay 10000ufury (.01 FURY)
FEE="10000"
```
...
To run the oracle from a Chainlink node, you need to configure an external adapter for interacting with fury. Instructions can be found at [chainlink-adapter-fury](https://github.com/Four4Two/external-adapters-js/tree/master/fury)
