...
Sentinel is a bot that will watch over a CDP on the fury blockchain and try to prevent it from being liquidated.

Sentinel will keep a CDP’s collateral ratio within a range by repaying (or withdrawing) debt.

## Get Started

Sentinel has been designed to be as reliable as possible. To further increase reliability, operators can run more than one copy of sentinel on different servers, using different fury nodes.

1. Install go
2. Install sentinel with `git clone <this repo>`, `make install`
   (`go install github.com/four4two/go-tools/sentinel` should also work)
