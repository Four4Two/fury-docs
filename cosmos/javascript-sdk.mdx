...
The Fury JavaScript SDK allows browsers and node.js clients to interact with Fury. Core functionality and query examples are in the `examples` folder.

- 🟡 client - client that implements Fury transactions and messages.
- 🟡 tx - Fury transaction types.
- 🟢 msg - Fury message types.
...
- Chain ID: fury-9
- REST API endpoint: https://api.data.fury.black
- Binance Chain mainnet REST API endpoint: https://dex.binance.org/

### Testnet

- Chain ID: fury-testnet-14000
- REST API endpoint: https://api.testnet.fury.black
...
- fury1r4v2zdhdalfj2ydazallqvrus9fkphmglhn6u6

**BTCB**

- bnb1xz3xqf4p2ygrw9lhp5g5df4ep4nd20vsywnmpr
- fury14qsmvzprqvhwmgql9fr0u3zv9n2qla8zhnm5pc

**BUSD**

- bnb10zq89008gmedc6rrwzdfukjk94swynd7dl97w8
- fury1hh4x3a4suu5zyaeauvmv7ypf7w9llwlfufjmuu

**XRPB**

- bnb15jzuvvg2kf0fka3fl2c8rx0kc3g6wkmvsqhgnh
- fury1c0ju5vnwgpgxnrktfnkccuth9xqc68dcdpzpas

## Client Setup

The client requires an address mnemonic and the url of Fury's REST api endpoint.
...
Collateralized debt positions have a minimum value of 10 USD and must be overcollateralized above a certain percentage threshold. Supported collateral coin types, their supply limits, and their minimum overcollateralization ratios can be checked at https://api.data.fury.black/cdp/parameters.
...
### Transferring funds to Fury

Fury supports secure transfers of BNB from Binance Chain to Fury and back via atomic swaps. The [bep3-deputy](https://github.com/binance-chain/bep3-deputy) process sits between the two blockchains and services swaps by relaying information back and forth.
...
1. Create an atomic swap on Binance Chain (note: atomic swaps are called HTLTs on Binance Chain)
   The deputy will automatically relay the swap from Fury to Binance Chain
   2a. Claim the atomic swap on Fury within swap's height span. Users have about 30 minutes to claim a swap after it is created.
   2b. Refund the atomic swap on Fury after the swap's height span - this happens if the swap is not claimed in time.

### Create swap

In order for an address to submit a swap on Fury it must hold pegged bnb tokens. The Binance Chain [docs](https://docs.binance.org/atomic-swap.html) describe how to create a swap on Binance Chain with BNB. When creating the swap on Binance Chain make sure to use the deputy's Binance Chain address as the swap's `recipient` and the deputy's Fury address as the swap's `senderOtherChain` or the deputy will not relay the swap.

Users create outgoing swaps on Fury by entering the deputy's Fury address in the recipient field. The following example is for the testnet. See full code examples for creating and claiming a swap between Fury and Binance Chain, see `incoming_swap.js` and `outgoing_swap.js` in the examples folder.
...
const recipient = 'fury1tfvn5t8qwngqd2q427za2mel48pcus3z9u73fl'; // deputy's address on fury testnet
...
// Calculate the expected swap ID on Fury
...
Note: swap height span must be within range 220-270, you can check the current mainet BEP3 module parameters at https://api.data.fury.black/bep3/parameters.
...
Fury is an open source project and contributions to the Fury JavaScript SDK are welcome. If you'd like contribute, please open an issue or pull request.
