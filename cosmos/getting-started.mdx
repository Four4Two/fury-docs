...
1. [Go 1.20](https://golang.org/doc/install) - used to build & install `futool`
...
## Getting The Fury Repository & Development Tools

Once you have the core tools installed & set up, its now time to get the following repositories from Github:

- [fury](https://github.com/Four4Two/fury)
  - Main Fury Repo that holds all modules
- [futool](https://github.com/Four4Two/futool)
  - Dev tools to interact with a Fury node
...
Fury is now running on your local machine! The endpoints are publically exposed to your localhost:
...
**NOTE**: Before Fury 9 (`v0.16`), the `fury` binary was two separate binaries. One for running the chain (`fud`) and a separate one for the cli (`kvcli`). If running older versions, you will need to use `kvcli` instead of `fury`.

**Example**:
Create a new account & fund it:

```
$ dfury keys add my-test-account
...
$ dfury tx bank send whale fury17fuj33p6y472j3kplsjdz966xlw6cdhwqzl8rl 1000000000ufury --gas-prices 0.01ufury -y
```
...
bash-5.1# fury --help
Daemon and CLI for the Fury blockchain.
...
For example, running a network with v0.21 of `fury`:

```
$ futool testnet bootstrap --fury.configTemplate v0.21
```
...
You can override the tag by setting the `FURY_TAG` environment variable. For instance running the master template with a fixed fury version of `v0.21.0`:

```
FURY_TAG=v0.21.0 futool testnet bootstrap
```

Note that at the time you're reading this, the master template's genesis and configuration may not be compatible with fury version v0.21.0.
...
Be sure to check out our guide on [developing for Fury's EVM](../ethereum/development).

**Disclaimer**: `futool` is beta software. 😊 Some of its features may not always work as expected. Generally, the commands used in this guide are frequently used enough to be reliable. If you run into any problems, please reach out to us on [Discord](https://discord.com/invite/kQzh3Uv) or by [opening an issue](https://github.com/Four4Two/futool/issues/new).
